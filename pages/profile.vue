<template>
  <div class="flex min-h-screen transition-colors bg-[#fdfdf6] dark:bg-tertiary">
    ...
    <div class="flex-1 md:ml-16 p-4 w-full">
      <div class="max-w-4xl mx-auto bg-white dark:bg-[#1f2937] rounded shadow-lg text-gray-700 dark:text-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-4 flex space-x-6 text-sm border-b border-gray-200 dark:border-gray-600">
          <div><span class="font-semibold">{{ posts?.length || 0 }}</span> publicaciones</div>
    ...
    <!-- Modal de creación de publicaciones -->
    <UModal v-model="isModalOpen" :overlay="true" prevent-close>
      <PostCreationModal @close="isModalOpen = false" />
    </UModal>
  </div>
</template>
...
import PostCreationModal from '@/components/posts/PostCreationModal.vue'
import UModal from '@/components/ui/UModal.vue'
import ProfilePostsSection from '@/components/posts/ProfilePostsSection.vue'
...
// ✅ SSR + CSR compatible
const { data: posts, pending: loading, error } = await useAsyncData('posts', fetchPosts)

console.log('🧪 posts.value:', posts.value) // ✅ Colócalo aquí

function handleSidebarClick(label: string) {
  if (label === 'Crear publicación') {
